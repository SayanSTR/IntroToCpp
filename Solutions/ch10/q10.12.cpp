 ...
 * order time of a node is simply the time at which drawing of the subtree rooted at that
 * node is finished. Based on this, the post-order number of a node is defined to be i if
 ...
 * number 1, the root the number 2^(h+1)-1, and the rightmost leaf the number 2^(h+1)-h-1.
 */
...
void treeLine(int h, float hb, float wb, float r_x, float r_y, int &n) {
    ...
    if(h==0) {
        ...
        Text numL(r_x-5, r_y-5, n); numL.setColor(COLOR("blue")); numL.imprint(); n++;
    }
}
